...
class Ct:
    def __init__(self,series_uid):
        ...
        self.vxSize_xyz = self.VxSize_xyz  # lowercase alias matching book's convention
...
def getCtAugmentedCandidate(
    ...
    if use_cache :
        ct_chunk, _, center_irc = getCtRawCandidate(series_uid,center_xyz,width_irc)
    else:
        ct = getCt(series_uid)
        ct_chunk, _, center_irc = ct.getRawCandidate(center_xyz, width_irc)
    ...
    return augmented_chunk[0], None, center_irc # we gave augmented chuck[0], why [0] ?
...
class LunaDataset(Dataset):
    def __init__(
        ...
        self.ben_list = \
            [x for x in self.positive_list if not x.isMal_bool]
        self.mal_list = \
            [x for x in self.positive_list if x.isMal_bool]
    ...
    def __len__(self):
        if self.ratio_int:
            return 50000
        else : return len(self.candidateInfo_list)
    ...
    def __getitem__(self,ind):
        ...
        if self.augmentation_dict:
            candidate_t, _, center_irc = getCtAugmentedCandidate(
            ...
            candidate_a, _, center_irc = getCtRawCandidate(
                candidateInfo_tup.series_uid,
                candidateInfo_tup.center_xyz,
                width_irc,
            )
            ...
            candidate_a, _, center_irc = ct.getRawCandidate(
                candidateInfo_tup.center_xyz,
                width_irc,
            )
            candidate_t = torch.from_numpy(candidate_a).to(torch.float32)
        ...
        label_bool = candidateInfo_tup.isNodule_bool
        if not label_bool:
            label_t[0] = True
            index_t = 0
        else:
...
class MalignantLunaDataset(LunaDataset):
    ...
    def __getitem__(self, ndx):
        ...
        label_t = torch.tensor([False, False], dtype=torch.long)
        if not candidateInfo_tup.isMal_bool:
            label_t[0] = True
            index_t = 0
        else:
            label_t[1] = True
