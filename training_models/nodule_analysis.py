...
# Project root is one level above training_models/
...
from dataset_processing.dsets import Luna2dSegmentationDataset
from dataset_processing.dsets import LunaDataset, getCt, getCandidateInfoDict, getCandidateInfoList, CandidateInfoTuple
from training_models.model_seg import UNetWrapper

import training_models.model_conv
...
class NoduleAnalysisApp:
    def __init__(self, sys_argv=None):
        ...
        parser.add_argument('--segmentation-path',
            ...
            nargs='?',
            default=str(_PROJECT_ROOT / 'models' / 'seg.best.state'),
        )
        ...
        parser.add_argument('--classification-path',
            ...
            nargs='?',
            default=str(_PROJECT_ROOT / 'models' / 'cls.best.state'),
        )
        ...
        parser.add_argument('--malignancy-path',
            help="Path to the saved malignancy model",
            nargs='?',
            default=None,
        )

        parser.add_argument('--train-if-missing',
            help="Train missing models in-process instead of failing.",
            action='store_true',
            default=False,
        )

        parser.add_argument('--epochs',
            help="Number of epochs to train if models are missing.",
            type=int,
            default=15,
        )
    ...
    def initModels(self):
        from training_models.checkpoint_manager import ensure_checkpoints
        ensure_checkpoints(self.cli_args)
        ...
        model_cls = getattr(training_models.model_conv, self.cli_args.cls_model)
        cls_model = model_cls()
        ...
        if self.cli_args.malignancy_path:
            model_cls = getattr(training_models.model_conv, self.cli_args.malignancy_model)
